...
class ngaC():
    ...
    def __init__(self,X=None,Y=''):
        ...
        if Y=='':
            ...
            Y+='/'
        self.ppr=Y
        with open(self.ppr+"ngaVoteLog.txt","w",encoding="utf_8_sig") as lll:
            #没有用lll.write('''<head><meta charset="utf-8"></head>\n''')
        ...
        try:
            ...
            if hds["Cookie"]=='':
                ...
                if ses.get(lnk,headers=hds).status_code==200:
                    with open(self.ppr+"ngack.txt","w",encoding="utf_8_sig") as fw:
                        fw.write(hds["Cookie"])
                else:
    ...
    #try:
            with open(self.ppr+"ngaResult.json","w",encoding="utf_8_sig") as fr:#保存用户投票记录到文件
    ...
    #def counting():
        ...
        ###############################################dd
                ...
                try:
                    ...
                    with open(self.ppr+"ngaIgnore_dict.txt","w",encoding="utf_8_sig") as fa:
                        for i in self.igfrom:
                            fa.write(json.dumps({"type":"n","from":i})+"\n")
                    with open(self.ppr+"ngaIgnore_dict.txt","a",encoding="utf_8_sig") as fa:      
                ...
                try:
                    ...
                    if cmdli[0]=="n" and cmdli[1].isdigit():
                        ...
                        raise NameError("参数不明")
                    with open(self.ppr+"ngaTranslate_dict.txt","w",encoding="utf_8_sig") as fa:
                        for i in range(len(self.trfrom)):
                            fa.write(json.dumps({"type":"n","from":self.trfrom[i],"to":self.trto[i]})+"\n")
                    with open(self.ppr+"ngaTranslate_dict.txt","a",encoding="utf_8_sig") as fa:      
        ...
        #############################################save
                ...
                if askconf=="q":
                    ...
                    with open(self.ppr+"给人看的结果.txt","w",encoding="utf_8_sig") as fwh:
                        ...
                        with open(self.ppr+"Result.json","w",encoding="utf_8_sig") as fw:
